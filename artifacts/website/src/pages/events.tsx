    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-white/10">
          ...
          <button onClick={onClose} className="p-1.5 rounded-full text-white/50 hover:text-white hover:bg-white/10 transition-colors" data-testid="btn-close-modal">
            <X size={18} />
          </button>
        </div>
        <div className="p-5 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2 text-sm">
            <div className="flex items-center gap-2.5 text-white/60"><Calendar size={14} className="text-primary shrink-0" />{formatDate(event.date)}</div>
            <div className="flex items-center gap-2.5 text-white/60"><Clock size={14} className="text-primary shrink-0" />{event.time}{event.endTime ? ` – ${event.endTime}` : ""}</div>
            <div className="flex items-center gap-2.5 text-white/60"><MapPin size={14} className="text-primary shrink-0" />{event.location}</div>
            {event.isRecurring && (
              <div className="flex items-center gap-2.5 text-white/60"><RefreshCw size={14} className="text-primary shrink-0" />{event.recurringPattern}</div>
            )}
          </div>
  ...
  return (
    <AppShell subtitle="Events">
      ...
      <div className="px-4 pb-4 flex flex-col gap-5">
        {recurring.length > 0 && (
          <div>
            <p className="text-white/50 text-xs font-semibold uppercase tracking-widest mb-2.5 flex items-center gap-1.5">
            ...
            <div className="flex flex-col gap-2">
              {recurring.map((event) => (
                <button
                  ...
                  className="w-full text-left flex items-start justify-between gap-3 p-4 rounded-2xl bg-card border border-white/10 hover:border-primary/40 transition-all"
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                      ...
                      <span className="text-white/40 text-[10px] flex items-center gap-1"><RefreshCw size={9} />{event.recurringPattern}</span>
                    </div>
                    <p className="text-white font-bold text-sm leading-snug line-clamp-2">{event.title}</p>
