...
const metroPort = parseInt(process.env.EXPO_PUBLIC_METRO_PORT || "8081", 10);
...
function setupSignalHandlers() {
  const cleanup = () => { killMetro(); process.exit(0); };
...
function metroUrl(pathPart) {
  return `http://localhost:${metroPort}${pathPart}`;
}
...
function getExpoPublicReplId() {
  return process.env.EXPO_PUBLIC_REPL_ID || process.env.REPL_ID || "";
}
...
async function startMetro(expoPublicDomain, expoPublicReplId) {
  ...
  const env = {
    ...process.env,
    EXPO_PUBLIC_DOMAIN: expoPublicDomain || "localhost",
  };
  if (expoPublicReplId) {
  ...
  metroProcess = spawn("pnpm", [
    "exec", "expo", "start",
    "--no-dev", "--minify", "--localhost",
  ...
  console.error("Metro failed to start within 60s on port " + metroPort);
  process.exit(1);
}
...
async function downloadBundle(platform, timestamp) {
  ...
  const output = path.join(
    "static-build", timestamp, "_expo", "static", "js", platform, "bundle.js",
  );
  console.log("Fetching " + platform + " bundle...");
  await downloadFile(url.toString(), output);
  const size = fs.statSync(output).size;
  console.log(platform + " bundle ready (" + formatSize(size) + ")");
}
...
function extractAssets(timestamp) {
  ...
  const bundles = {
    ios: fs.readFileSync(
      path.join(staticBuild, timestamp, "_expo", "static", "js", "ios", "bundle.js"), "utf-8",
    ),
    android: fs.readFileSync(
      path.join(staticBuild, timestamp, "_expo", "static", "js", "android", "bundle.js"), "utf-8",
    ),
  };
...
async function copyAssets(assets, timestamp) {
  ...
  const copyPromises = assets.map(async (asset) => {
    ...
    const outputDir = path.join(
      projectRoot, "static-build", timestamp, "_expo", "static", "js", asset.relativePath,
    );
    ...
    try {
      ...
      failures.push({ filename: asset.filename, error: error.message, url: asset.originalPath });
    }
  });
  await Promise.all(copyPromises);
  if (failures.length > 0) {
    const errorMsg = "Failed to copy " + failures.length + " asset(s):\n" +
...
function updateBundleUrls(timestamp, baseUrl) {
  const updateForPlatform = (platform) => {
    const bundlePath = path.join(
      projectRoot, "static-build", timestamp, "_expo", "static", "js", platform, "bundle.js",
    );
...
function updateManifests(manifests, timestamp, baseUrl, assetsByHash) {
  const updateForPlatform = (platform, manifest) => {
    ...
    const hostPart = baseUrl.replace("https://", "").replace("http://", "");
    manifest.extra.expoClient.hostUri = hostPart + "/" + platform;
    manifest.extra.expoGo.debuggerHost = hostPart + "/" + platform;
...
function printSummary(timestamp, domain) {
  const staticBuild = path.join(projectRoot, "static-build");
  const outputDir = path.join(staticBuild, timestamp);
  const iosBundle = path.join(outputDir, "_expo", "static", "js", "ios", "bundle.js");
  const androidBundle = path.join(outputDir, "_expo", "static", "js", "android", "bundle.js");

  const iosSize = fs.existsSync(iosBundle) ? fs.statSync(iosBundle).size : 0;
  const androidSize = fs.existsSync(androidBundle) ? fs.statSync(androidBundle).size : 0;
  const totalSize = iosSize + androidSize;
  ...
  console.log("  iOS bundle: " + formatSize(iosSize));
  console.log("  Android:    " + formatSize(androidSize));
  console.log("  Total:      " + formatSize(totalSize));
  console.log("  Platforms:  ios, android");

  if (domain) {
    console.log("  Deploy to:  https://" + domain);
    console.log("  Serve with: pnpm --filter @workspace/mobile run serve");
  } else {
    console.log("  Local build (relative URLs)");
    console.log("  Set EXPO_PUBLIC_DOMAIN for deployment builds");
    console.log("  Serve with: pnpm --filter @workspace/mobile run serve");
  }
  console.log("═══════════════════════════════════════");
  console.log("");
...
async function main() {
  const isLocal = !process.env.EXPO_PUBLIC_DOMAIN &&
    !process.env.REPLIT_INTERNAL_APP_DOMAIN &&
    !process.env.REPLIT_DEV_DOMAIN;

  if (isLocal) {
    console.log("Building for local development (no deployment domain set)");
    console.log("Set EXPO_PUBLIC_DOMAIN for production deployment builds");
    console.log("");
  }
  ...
  await startMetro(domain || "localhost", expoPublicReplId);
  ...
  const assetCount = await copyAssets(assets, timestamp);
  if (assetCount > 0) {
